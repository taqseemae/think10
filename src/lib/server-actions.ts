...
export const createBookingFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      console.error('[Think10] Zoom API Error:', zoomError?.message);
      throw new Error(`Zoom Meeting creation failed: ${zoomError?.message || 'Check Zoom env variables in Coolify.'}`);
    }
