...
export const Route = createFileRoute("/experts/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        { title: `${e?.name ?? "Expert"} | Think10 advisor` },
        { name: "description", content: e ? `${e.role} — ${e.bio}` : "Think10 advisor" },
        { property: "og:title", content: e?.name ?? "Think10 advisor" },
        { property: "og:url", content: `/experts/${e?.slug ?? ""}` },
      ],
      links: [{ rel: "canonical", href: `/experts/${e?.slug ?? ""}` }],
      scripts: e
        ? [
            {
