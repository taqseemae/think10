...
function Overview() {
  ...
  const handleQuickChatSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (q) {
      router.push(`/dashboard/zyne?q=${encodeURIComponent(q)}`);
    }
  };
  ...
  const getNextBestAction = () => {
    ...
    if (activeBooking) {
      return {
        ...
        action: () => router.push("/dashboard/sessions"),
        accent: "emerald",
      };
    } else if (overdueTask) {
      return {
        ...
        action: () => router.push("/dashboard/action-plans"),
        accent: "yellow",
      };
    ...
    return {
      ...
      action: () => router.push("/dashboard/zyne"),
      accent: "emerald",
    };
  };
          ...
          <div className="rounded-2xl border border-[color:var(--t10-border)] bg-white p-6 shadow-sm">
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              {["Price DTC vs noon", "FBA fee audit", "Ramadan planning"].map((q) => (
                <Link
                  key={q}
                  href={`/dashboard/zyne?q=${encodeURIComponent(q)}`}
