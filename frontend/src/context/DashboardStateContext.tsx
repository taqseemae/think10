  ...
  const setRole = (newRole: UserRole) => {
    ...
    if (currentUser) {
      ...
      import("@/lib/server-actions").then(({ updateUserPlanFn }) => {
        updateUserPlanFn({ data: { uid: currentUser.uid, plan: { role: newRole } } }).catch(console.error);
      });
    }
  ...
  const updateHealthScores = (scores: Partial<HealthScores>) => {
    ...
    if (currentUser) {
      ...
      import("@/lib/server-actions").then(({ updateHealthScoresFn }) => {
        updateHealthScoresFn({ data: { uid: currentUser.uid, healthScores: newScores } }).catch(console.error);
      });
    }
  };
...
Do not use markdown blocks for the JSON. Just output raw JSON.`;
  ...
  const cancelBooking = (bookingId: string) => {
    import("@/lib/server-actions").then(({ updateBookingStatusFn }) => {
      updateBookingStatusFn({ data: { bookingId, status: "CANCELLED" } })
  ...
  const triggerServiceRecovery = (bookingId: string, type: "TECH_FAILURE" | "NO_SHOW") => {
    import("@/lib/server-actions").then(({ updateBookingStatusFn }) => {
      updateBookingStatusFn({ data: { bookingId, status: type } })
  ...
  const completeCall = (bookingId: string, rating: number, feedback: string) => {
    import("@/lib/server-actions").then(({ updateBookingStatusFn }) => {
      updateBookingStatusFn({ data: { bookingId, status: "COMPLETED" } })
